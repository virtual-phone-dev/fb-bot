...
import asyncio, json, os, random, itertools, sys, traceback, requests
...
with open("liste-pages-congo.json", "r", encoding="utf-8") as f:
    PAGES = json.load(f)
#URLS = [p["url"] for p in PAGES] # Extraire directement les URL


# Charger la liste des commentaires
with open("phrase-a-commenter.json", "r", encoding="utf-8") as f:
    COMMENTS = json.load(f)

# commentaires avec le premier mot dupliquer
with open("phrase-a-commenter2.json", "r", encoding="utf-8") as f:
...
def load_commented_posts():
    ...
    filename = JSON_FILE_COMMENTED_POSTS

    # si fichier n'existe pas → créer avec []
    if not os.path.exists(filename):
        with open(filename, "w", encoding="utf-8") as f:
            json.dump([], f)
        return set()

    # si fichier existe mais vide → écrire []
    ...
    try:
        ...
        # si contenu cassé → reset automatique
        with open(filename, "w", encoding="utf-8") as f:
            json.dump([], f)
        return set()
